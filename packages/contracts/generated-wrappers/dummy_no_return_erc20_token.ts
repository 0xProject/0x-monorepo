...
export type DummyNoReturnERC20TokenEventArgs =
    | DummyNoReturnERC20TokenTransferEventArgs
    | DummyNoReturnERC20TokenApprovalEventArgs;

export enum DummyNoReturnERC20TokenEvents {
    Transfer = 'Transfer',
    Approval = 'Approval',
}
...
export class DummyNoReturnERC20TokenContract extends BaseContract {
    public name = {
        async callAsync(
        ...
        > {
            const self = this as any as DummyNoReturnERC20TokenContract;
    ...
    ): Promise<DummyNoReturnERC20TokenContract> {
        if (_.isUndefined(artifact.compilerOutput)) {
            throw new Error('Compiler output not found in the artifact file');
        }
        const bytecode = artifact.compilerOutput.evm.bytecode.object;
        const abi = artifact.compilerOutput.abi;
        return DummyNoReturnERC20TokenContract.deployAsync(bytecode, abi, provider, txDefaults, _name,
_symbol,
_decimals,
_totalSupply
);
...
_totalSupply
        ...
        logUtils.log(`DummyNoReturnERC20Token successfully deployed at ${txReceipt.contractAddress}`);
        const contractInstance = new DummyNoReturnERC20TokenContract(abi, txReceipt.contractAddress as string, provider, txDefaults);
...
_totalSupply
    ...
    constructor(abi: ContractAbi, address: string, provider: Provider, txDefaults?: Partial<TxData>) {
        super('DummyNoReturnERC20Token', abi, address, provider, txDefaults);
