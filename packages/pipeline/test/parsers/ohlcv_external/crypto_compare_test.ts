...
describe('ohlcv_external parser (Crypto Compare)', () => {
    describe('parseRecords', () => {
        ...
        entity.exchange = 'CCCAGG';
        entity.fromSymbol = pair.fromSymbol;
        entity.toSymbol = pair.toSymbol;
        ...
        entity.source = 'CryptoCompare';
        entity.observedTimestamp = scraped;

        it('converts Crypto Compare OHLCV records to OHLCVExternal entity', () => {
            const input = [
              record,
              merge(record, { time: 300 }),
              merge(record, { time: 400 }),
            ];
            const expected = [
              entity,
            ...
            const actual = parseRecords(input, pair, scraped);
        ...
        it('ignores records with no exchange specified', () => {
          const input = [
            record,
            merge(record, { time: 300, exchange: undefined }),
            merge(record, { time: 400 }),
          ];
          const expected = [
            entity,
