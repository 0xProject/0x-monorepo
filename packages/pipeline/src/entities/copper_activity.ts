...
export class CopperActivity {
    ...
    @Column({ name: 'parent_type', type: 'varchar' }) public parentType!: string;

    // join with CopperActivityType
    @Index()
    @Column({ name: 'type_id', type: 'bigint', transformer: numberToBigIntTransformer })
    public typeId!: number;
    @Column({ name: 'type_category', type: 'varchar' }) public typeCategory!: string;
    ...
    @Column({ name: 'new_value_name', type: 'varchar', nullable: true }) public newValueName?: string;
