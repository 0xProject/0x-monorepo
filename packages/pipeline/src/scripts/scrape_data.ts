...
export const scrapeDataScripts = {
    ...
    async asyncGetHistoricalPrices(
        fromSymbol: string,
        toSymbol: string,
        fromTimestamp: number,
        toTimestamp: number,
    ): Promise<HistoricalPriceResponse> {
        const daysInQueryPeriod = Math.round((toTimestamp - fromTimestamp) / (86400 * 1000));
        var parsedParams = {
            fsym: fromSymbol,
            tsym: toSymbol,
...
function _scrapeHistoricalPricesToDB(token: any, fromTimestamp: number, toTimestamp: number): any {
    return (cb: () => void) => {
        scrapeDataScripts
            .asyncGetHistoricalPrices(token, BASE_SYMBOL, fromTimestamp, toTimestamp)
            .then((data: any) => {
                if (data['Response'] == 'Success') {
                    const parsedHistoricalPrices: any = [];
                    for (const rawPrice of data['Data']) {
                        const parsedHistoricalPrice = typeConverters.convertLogHistoricalPricesToHistoricalPricesObject(
                            rawPrice,
                        );
