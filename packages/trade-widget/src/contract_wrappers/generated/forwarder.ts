...
export class ForwarderContract extends BaseContract {
    public fillOrder = {
        async sendTransactionAsync(
            ...
            [order, signature] = BaseContract._transformABIData(
                inputAbi,
                [order, signature],
                BaseContract._bigNumberToString.bind(this),
            );
            const encodedData = this._ethersInterface.functions.fillOrder(order, signature).data;
            const txDataWithDefaults = await self._applyDefaultsToTxDataAsync(
                {
                    ...txData,
                    data: encodedData,
                },
                self.fillOrder.estimateGasAsync.bind(self, order, signature),
            );
        ...
        getABIEncodedTransactionData(
            ...
            const abiEncodedTransactionData = this._ethersInterface.functions.fillOrder(order, signature).data;
        ...
        async callAsync(
            ...
            const encodedData = self._ethersInterface.functions.fillOrder(order, signature).data;
    ...
    public getOrderHash = {
        async callAsync(
            ...
            [order] = BaseContract._transformABIData(inputAbi, [order], BaseContract._bigNumberToString.bind(this));
            const encodedData = self._ethersInterface.functions.getOrderHash(order).data;
            const callData = await self._applyDefaultsToTxDataAsync({
                data: encodedData,
            });
            const rawCallResult = await self._web3Wrapper.callAsync(callData, defaultBlock);
