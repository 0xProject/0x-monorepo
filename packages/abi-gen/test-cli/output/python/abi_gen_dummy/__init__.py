...
class NonPureFunctionMethod(ContractMethod):
    """Various interfaces to the nonPureFunction method."""
    ...
    def validate_and_normalize_inputs(self, _id: int, some_value: int):
        """Validate the inputs to the nonPureFunction method."""
        self.validator.assert_valid(
            method_name="nonPureFunction",
            parameter_name="id",
            argument_value=_id,
        )
        # safeguard against fractional inputs
        ...
        return (_id, some_value)

    def call(
        self, _id: int, some_value: int, tx_params: Optional[TxParams] = None
        ...
        :returns: the return value of the underlying method.
        """
        (_id, some_value) = self.validate_and_normalize_inputs(_id, some_value)
        tx_params = super().normalize_tx_params(tx_params)
        return self.underlying_method(_id, some_value).call(
            tx_params.as_dict()
        )
    ...
    def send_transaction(
        ...
        (_id, some_value) = self.validate_and_normalize_inputs(_id, some_value)
        tx_params = super().normalize_tx_params(tx_params)
        return self.underlying_method(_id, some_value).transact(
            tx_params.as_dict()
        )
    ...
    def estimate_gas(
        ...
        (_id, some_value) = self.validate_and_normalize_inputs(_id, some_value)
        tx_params = super().normalize_tx_params(tx_params)
        return self.underlying_method(_id, some_value).estimateGas(
            tx_params.as_dict()
        )
...
class SomeMappingMethod(ContractMethod):
    """Various interfaces to the someMapping method."""
    ...
    def validate_and_normalize_inputs(self, index_0: int):
        """Validate the inputs to the someMapping method."""
        self.validator.assert_valid(
            method_name="someMapping",
            parameter_name="index_0",
            argument_value=index_0,
        )
        # safeguard against fractional inputs
...
class AbiGenDummy:
    ...
    non_pure_function: NonPureFunctionMethod
    """Constructor-initialized instance of
    :class:`NonPureFunctionMethod`.
    """
    ...
    some_mapping: SomeMappingMethod
    """Constructor-initialized instance of
    :class:`SomeMappingMethod`.
    """
    ...
    def __init__(
        ...
        self.non_pure_function = NonPureFunctionMethod(
            provider, contract_address, functions.nonPureFunction, validator
        )
        ...
        self.some_mapping = SomeMappingMethod(
            provider, contract_address, functions.someMapping, validator
        )
