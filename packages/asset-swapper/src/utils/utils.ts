...
export const utils = {
    ...
    isOrderTakerFeePayableWithMakerAsset<T extends Order>(order: T): boolean {
        return (
            order.takerFeeAssetData === NULL_BYTES ||
            utils.isAssetDataEquivalent(order.takerFeeAssetData, order.makerAssetData)
        );
    },
    isOrderTakerFeePayableWithTakerAsset<T extends Order>(order: T): boolean {
        return (
            order.takerFeeAssetData === NULL_BYTES ||
            utils.isAssetDataEquivalent(order.takerFeeAssetData, order.takerAssetData)
        );
    },
