...
export class Compiler {
    ...
    private _recursivelyGatherDependencySources(
        ...
        for (const importStatementMatch of importStatementMatches) {
            ...
            //
            // AMEND(jvanoers): Normalise path separators and use path.join to avoid hack
            //
            // Previous solution used path.resolve and associated hack
            // in order to trick path resolution using path.resolve. Instead, we use
            // path.join to avoid absolute path resolution.
            //
            //      path.join("a/b/c/Foo.sol", "../../d/e/Bar.sol") === "a/d/e/Bar.sol"
            //
            // However, path resolves directories according to the native path separator:
            //      "a/b/Foo.sol" on *nix
            //      "a\\b\\Foo.sol" on Windows
            //
            // This resolution conflicts with the format in fullSources (generated by solc
            // and earlier inputs in the toolchain). fullSources may also include absolute
            // imports that use *nix path separators "zeppelin-solidity/foo/Bar.sol"
            // To hack around this, we normalise all string comparison to using the same
            // path separator: '/'.
            //
            // AMEND(jvanoers): Support for Absolute resolutions
            //
            // Distinguish between relative imports ("./Foo.sol") and absolute imports
            // ("github.com/foo/Bar.sol", "zeppelin-solidity/foo/Bar.sol", "http://foo.bar/Baz.sol")
            // by string comparing to '.' and not attempting to resolve a contract folder.

            const lastPathSeparatorPos = contractPath.split(path.sep).join('/').lastIndexOf('/');
            const isRelativeImport = importPath.startsWith('.');

            const contractFolder = lastPathSeparatorPos === -1 || !isRelativeImport ? '' : contractPath.slice(0, lastPathSeparatorPos + 1);
            importPath = path.join(contractFolder, importPath).split(path.sep).join('/');

            if (_.isUndefined(sourcesToAppendTo[importPath])) {
                const normalisedSources:  { [sourceName: string]: { id: number } } = Object.entries(fullSources)
                    .reduce((prev, [oldPath, value]) => Object.assign(prev, { [oldPath.split(path.sep).join('/')]: value }), {});

                sourcesToAppendTo[importPath] = { id: normalisedSources[importPath].id };
