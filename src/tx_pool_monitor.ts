...
import {etherdeltaAbi} from './etherdelta';
...
export class TxPoolMonitor {
    ...
    private _lock = false;
    private _typesBySig: {[sig: string]: string[]};
    private _signatureBySig: {[sig: string]: string};
    constructor(web3Wrapper: Web3Wrapper) {
        this._web3Wrapper = web3Wrapper;
        this._typesBySig = {};
        this._signatureBySig = {};
        for (const fAbi of _.filter(etherdeltaAbi, {type: 'function', constant: false})) {
            const types = _.map(fAbi.inputs, input => input.type);
            const hrTypes = _.map(fAbi.inputs, input => `${input.type} ${input.name}`);
            const signature = fAbi.name + '(' + types.join(',') + ')';
            const hrSignature = fAbi.name + '(' + hrTypes.join(',') + ')';
            const sig = this._web3Wrapper.sha3(signature).substr(0, 10);
            this._typesBySig[sig] = types;
            this._signatureBySig[sig] = hrSignature;
        }
    }
    public watch() {
        this.watchBlocks();
    ...
    private async stepAsync(): Promise<void> {
        ...
        txs = _.filter(txs, tx => tx.to === ETHERDELTA);
    ...
    private printTxDetails(tx: Web3.Transaction): void {
        ...
        const signature = input.substr(0, 10);
        const params = input.substr(10);
        const types = this._typesBySig[signature];
        console.log(this._signatureBySig[signature]);
