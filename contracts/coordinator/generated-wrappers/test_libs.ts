...
export class TestLibsContract extends BaseContract {
    ...
    public publicGetCoordinatorApprovalHash = {
        async callAsync(
        ...
        > {
            const self = this as any as TestLibsContract;
            const encodedData = self._strictEncodeArguments('publicGetCoordinatorApprovalHash((bytes32,bytes,uint256))', [approval
        ]);
            const callDataWithDefaults = await BaseContract._applyDefaultsToTxDataAsync(
                {
                    to: self.address,
            ...
            const rawCallResult = await self._web3Wrapper.callAsync(callDataWithDefaults, defaultBlock);
            BaseContract._throwIfRevertWithReasonCallResult(rawCallResult);
            const abiEncoder = self._lookupAbiEncoder('publicGetCoordinatorApprovalHash((bytes32,bytes,uint256))');
            // tslint:disable boolean-naming
            const result = abiEncoder.strictDecodeReturnValue<string
        >(rawCallResult);
            // tslint:enable boolean-naming
