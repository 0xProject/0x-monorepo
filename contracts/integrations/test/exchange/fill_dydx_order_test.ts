...
blockchainTests.resets('Exchange fills dydx orders', env => {
    ...
    before(async () => {
        ...
        const bridgeProxyAssetData = deployment.assetDataEncoder.ERC20Bridge(testContract.address, testContract.address, bridgeData).getABIEncodedTransactionData();
    ...
    describe('fillOrder', () => {
        const verifyEvents = (logs: (LogWithDecodedArgs<DecodedLogArgs> | LogEntry)[]): void => {
            ...
            const fillEvent = _.find(logs, (log) => {return (log as any).event === 'Fill'}) as LogWithDecodedArgs<DecodedLogArgs>;
            ...
            const dydxDepositEvent = _.find(logs, (log) => {return (log as any).event === 'OperateAction' && (log as any).args.actionType == DydxBridgeActionType.Deposit}) as LogWithDecodedArgs<DecodedLogArgs>;
            const amountDepositedIntoDydx = dydxDepositEvent.args.amountValue;

            // Extract amount withdrawn from dydx to taker.
            const dydxWithdrawEvent = _.find(logs, (log) => {return (log as any).event === 'OperateAction' && (log as any).args.actionType == DydxBridgeActionType.Withdraw}) as LogWithDecodedArgs<DecodedLogArgs>;
