    ...
    */
    ...
    describe('this is it', () => {
        it('should be owned by ZeroExGovernor', async () => {
            ...
            console.log('Fetching Final Staking Params...');
            ...
            // Validate final staking params
            const stakingParamsFinalExpected = {
                ...stakingParamsInit,
                epochDurationInSeconds: new BigNumber(oneWeekInSeconds)
            };
            expect(stakingParamsFinal, 'Final staking params').to.deep.equal(stakingParamsFinalExpected);

            // Validate end of epoch
            console.log('Fetching & Validating New Epoch End Time...');
            const epochBeginTime = await stakingContract.currentEpochStartTimeInSeconds().callAsync();
            const epochEndTime = await stakingContract.getCurrentEpochEarliestEndTimeInSeconds().callAsync();
            expect(epochEndTime, 'Epoch End Time').to.bignumber.equal(epochBeginTime.plus(oneWeekInSeconds));

            // Fast-Forward to end of epoch
            console.log('Fast-Forwarding to end of epoch...');
            await env.web3Wrapper.increaseTimeAsync(oneWeekInSeconds + 1);
            await env.web3Wrapper.mineBlockAsync();

            // Check that epoch can be ended.
            console.log('Testing Epoch Finalization...');
            await stakingContract.endEpoch().callAsync();

            // Alert the user how awesome they are
