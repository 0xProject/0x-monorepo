...
import { blockchainTests, constants, toBaseUnitAmount } from '@0x/contracts-test-utils';
...
blockchainTests.fork('Mainnet Balancer bridge tests', env => {
    ...
    blockchainTests.resets('Can trade with two-asset pool', () => {
        it('successfully exchanges WETH for USDC', async () => {
            ...
            // Fund the Bridge
            const weth = new ERC20TokenContract(wethAddress, env.provider, env.txDefaults);
            await weth
        ...
        it('successfully exchanges USDC for WETH', async () => {
            ...
            // Fund the Bridge
            const usdc = new ERC20TokenContract(usdcAddress, env.provider, env.txDefaults);
            await usdc
