    ...
    DeploymentConstants
{
        ...
        returns (bytes4 success)
    {
        ...
        require(state.path.length >= 2, "PlasmaswapBridge/PATH_LENGTH_MUST_BE_AT_LEAST_TWO");
        require(state.path[state.path.length - 1] == toTokenAddress, "PlasmaswapBridge/LAST_ELEMENT_OF_PATH_MUST_MATCH_OUTPUT_TOKEN");
        ...
        // Grant the Plasmaswap router an allowance.
        LibERC20Token.approveIfBelow(
            state.path[0],
            _getPlasmaswapRouterAddress(),
            state.fromTokenBalance
        );

        // Buy as much `toTokenAddress` token with `fromTokenAddress` token
        // and transfer it to `to`.
        IPlasmaswapRouter router = IPlasmaswapRouter(_getPlasmaswapRouterAddress());
