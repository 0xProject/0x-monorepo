...
class ERC20Wrapper(ContractWrapper):
    ...
    def transfer(
        ...
        value = int(value)
        func = self._erc20(token_address).functions.transfer(
            to, value
        )
    ...
    def extract_transfer_event(
        self,
        token_address,
        tx_hash,
    ):
        token_address = self._validate_and_checksum_address(token_address)
        return self._erc20(token_address).Transfer().processReceipt(tx_hash)

    def extract_approval_event(
        self,
        token_address,
        tx_hash,
    ):
        token_address = self._validate_and_checksum_address(token_address)
        return self._erc20(token_address).Approval().processReceipt(tx_hash)
